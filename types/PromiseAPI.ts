...
export class PromiseEmailControllerApi {
     ...
     */
    public sendEmailSourceOptional(inboxId: string, sendEmailOptions: SendEmailOptions, useDomainPool?: boolean, options?: Configuration): Promise<void> {
        const result = this.api.sendEmailSourceOptional(inboxId, sendEmailOptions, useDomainPool, options);
        return result.toPromise();
    }
...
export class PromiseInboxControllerApi {
     ...
     */
    public createInbox(emailAddress?: string, tags?: Array<string>, name?: string, description?: string, useDomainPool?: boolean, favourite?: boolean, expiresAt?: Date, expiresIn?: number, allowTeamAccess?: boolean, inboxType?: 'HTTP_INBOX' | 'SMTP_INBOX', options?: Configuration): Promise<InboxDto> {
        const result = this.api.createInbox(emailAddress, tags, name, description, useDomainPool, favourite, expiresAt, expiresIn, allowTeamAccess, inboxType, options);
        return result.toPromise();
    }
     ...
     */
    public getEmails(inboxId: string, aliasForLimitAssessedFirstBeforeAssessingAnyPassedLimit?: number, limit?: number, sortTheResultsByReceivedDateAndDirectionASCOrDESC?: 'ASC' | 'DESC', retryTimeout?: number, delayTimeout?: number, minCount?: number, unreadOnly?: boolean, before?: Date, since?: Date, options?: Configuration): Promise<Array<EmailPreview>> {
        const result = this.api.getEmails(inboxId, aliasForLimitAssessedFirstBeforeAssessingAnyPassedLimit, limit, sortTheResultsByReceivedDateAndDirectionASCOrDESC, retryTimeout, delayTimeout, minCount, unreadOnly, before, since, options);
        return result.toPromise();
    }
