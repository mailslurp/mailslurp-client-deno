...
export class PromiseWebhookControllerApi {
    ...
    public getAllWebhooks(page?: number, size?: number, sort?: 'ASC' | 'DESC', searchFilter?: string, since?: Date, inboxId?: string, phoneId?: string, before?: Date, health?: 'HEALTHY' | 'UNHEALTHY', eventType?: 'EMAIL_RECEIVED' | 'NEW_AI_TRANSFORM_RESULT' | 'NEW_EMAIL' | 'NEW_CONTACT' | 'NEW_ATTACHMENT' | 'EMAIL_OPENED' | 'EMAIL_READ' | 'DELIVERY_STATUS' | 'BOUNCE' | 'BOUNCE_RECIPIENT' | 'NEW_SMS' | 'NEW_GUEST_USER', url?: string, options?: Configuration): Promise<PageWebhookProjection> {
        const result = this.api.getAllWebhooks(page, size, sort, searchFilter, since, inboxId, phoneId, before, health, eventType, url, options);
        return result.toPromise();
    }
     ...
     * @param overrideAuth 
     */
    public updateWebhook(webhookId: string, createWebhookOptions: CreateWebhookOptions, inboxId?: string, phoneNumberId?: string, overrideAuth?: boolean, options?: Configuration): Promise<WebhookDto> {
        const result = this.api.updateWebhook(webhookId, createWebhookOptions, inboxId, phoneNumberId, overrideAuth, options);
        return result.toPromise();
    }
