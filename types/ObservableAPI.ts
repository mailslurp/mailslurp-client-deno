...
export class ObservablePhoneControllerApi {
     ...
     * @param favourite Optionally filter results for favourites only
     */
    public getPhoneNumbers(phoneCountry?: 'US' | 'GB' | 'AU', page?: number, size?: number, sort?: 'ASC' | 'DESC', since?: Date, before?: Date, search?: string, include?: Array<string>, favourite?: boolean, options?: Configuration): Observable<PagePhoneNumberProjection> {
...
export class ObservableSseControllerApi {
    private requestFactory: SseControllerApiRequestFactory;
    private responseProcessor: SseControllerApiResponseProcessor;
    private configuration: Configuration;
    ...
    public constructor(
        ...
        this.configuration = configuration;
        this.requestFactory = requestFactory || new SseControllerApiRequestFactory(configuration);
        this.responseProcessor = responseProcessor || new SseControllerApiResponseProcessor();
    }

    /**
     * @param xApiKey 
     */
    public streamEvents(xApiKey: string, options?: Configuration): Observable<Array<string>> {
        const requestContextPromise = this.requestFactory.streamEvents(xApiKey, options);
        ...
        return middlewarePreObservable.pipe(mergeMap((ctx: RequestContext) => this.configuration.httpApi.send(ctx))).
            pipe(mergeMap((response: ResponseContext) => {
                ...
                return middlewarePostObservable.pipe(map((rsp: ResponseContext) => this.responseProcessor.streamEvents(rsp)));
            }));
    }
