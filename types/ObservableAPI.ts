...
export class ObservableWaitForControllerApi {
     ...
     * @param unreadOnly Optional filter for unread only
     */
    public waitForMatchingEmail(matchOptions: MatchOptions, count?: number, delay?: number, inboxId?: string, since?: Date, sort?: 'ASC' | 'DESC', timeout?: number, unreadOnly?: boolean, options?: Configuration): Observable<Array<EmailPreview> | void> {
        const requestContextPromise = this.requestFactory.waitForMatchingEmail(matchOptions, count, delay, inboxId, since, sort, timeout, unreadOnly, options);
