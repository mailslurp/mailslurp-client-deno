...
export class ObservableEmailControllerApi {
     ...
     */
    public sendEmailSourceOptional(inboxId: string, sendEmailOptions: SendEmailOptions, useDomainPool?: boolean, options?: Configuration): Observable<void> {
        const requestContextPromise = this.requestFactory.sendEmailSourceOptional(inboxId, sendEmailOptions, useDomainPool, options);
...
export class ObservableInboxControllerApi {
     ...
     */
    public createInbox(emailAddress?: string, tags?: Array<string>, name?: string, description?: string, useDomainPool?: boolean, favourite?: boolean, expiresAt?: Date, expiresIn?: number, allowTeamAccess?: boolean, inboxType?: 'HTTP_INBOX' | 'SMTP_INBOX', options?: Configuration): Observable<InboxDto> {
        const requestContextPromise = this.requestFactory.createInbox(emailAddress, tags, name, description, useDomainPool, favourite, expiresAt, expiresIn, allowTeamAccess, inboxType, options);
    ...
    public getEmails(inboxId: string, aliasForLimitAssessedFirstBeforeAssessingAnyPassedLimit?: number, limit?: number, sortTheResultsByReceivedDateAndDirectionASCOrDESC?: 'ASC' | 'DESC', retryTimeout?: number, delayTimeout?: number, minCount?: number, unreadOnly?: boolean, before?: Date, since?: Date, options?: Configuration): Observable<Array<EmailPreview>> {
        const requestContextPromise = this.requestFactory.getEmails(inboxId, aliasForLimitAssessedFirstBeforeAssessingAnyPassedLimit, limit, sortTheResultsByReceivedDateAndDirectionASCOrDESC, retryTimeout, delayTimeout, minCount, unreadOnly, before, since, options);
