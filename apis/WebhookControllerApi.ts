...
export class WebhookControllerApiRequestFactory extends BaseAPIRequestFactory {
     ...
     */
    public async getTestWebhookPayload(options?: Configuration): Promise<RequestContext> {
        let config = options || this.configuration;
...
export class WebhookControllerApiResponseProcessor {
     ...
     * @throws ApiException if the response code was not in [200, 299]
     */
     public async getTestWebhookPayload(response: ResponseContext): Promise<WebhookPayload > {
        const contentType = ObjectSerializer.normalizeMediaType(response.headers["content-type"]);
        if (isCodeInRange("200", response.httpStatusCode)) {
            const body: WebhookPayload = ObjectSerializer.deserialize(
                ObjectSerializer.parse(await response.body.text(), contentType),
                "WebhookPayload", ""
            ) as WebhookPayload;
            return body;
