...
export class WebhookControllerApiRequestFactory extends BaseAPIRequestFactory {
     ...
     */
    public async getAllWebhookResults(page?: number, searchFilter?: string, size?: number, sort?: 'ASC' | 'DESC', options?: Configuration): Promise<RequestContext> {
        let config = options || this.configuration;
     ...
     * @param sort Optional createdAt sort direction ASC or DESC
     */
    public async getAllWebhooks(page?: number, searchFilter?: string, size?: number, sort?: 'ASC' | 'DESC', options?: Configuration): Promise<RequestContext> {
        let config = options || this.configuration;
     ...
     * @param sort Optional createdAt sort direction ASC or DESC
     */
    public async getInboxWebhooksPaginated(inboxId: string, page?: number, searchFilter?: string, size?: number, sort?: 'ASC' | 'DESC', options?: Configuration): Promise<RequestContext> {
        let config = options || this.configuration;
     ...
     */
    public async getTestWebhookPayload(eventName?: 'EMAIL_RECEIVED' | 'NEW_EMAIL' | 'NEW_CONTACT' | 'NEW_ATTACHMENT' | 'EMAIL_OPENED', options?: Configuration): Promise<RequestContext> {
        let config = options || this.configuration;
        ...
        if (eventName !== undefined) {
            requestContext.setQueryParam("eventName", ObjectSerializer.serialize(eventName, "'EMAIL_RECEIVED' | 'NEW_EMAIL' | 'NEW_CONTACT' | 'NEW_ATTACHMENT' | 'EMAIL_OPENED'", ""));
        }
     ...
     * @param sort Optional createdAt sort direction ASC or DESC
     */
    public async getWebhookResults(webhookId: string, page?: number, searchFilter?: string, size?: number, sort?: 'ASC' | 'DESC', options?: Configuration): Promise<RequestContext> {
        let config = options || this.configuration;
...
export class WebhookControllerApiResponseProcessor {
     ...
     * @throws ApiException if the response code was not in [200, 299]
     */
     public async getWebhookResult(response: ResponseContext): Promise<WebhookResultEntity > {
        const contentType = ObjectSerializer.normalizeMediaType(response.headers["content-type"]);
        if (isCodeInRange("200", response.httpStatusCode)) {
            const body: WebhookResultEntity = ObjectSerializer.deserialize(
                ObjectSerializer.parse(await response.body.text(), contentType),
                "WebhookResultEntity", ""
            ) as WebhookResultEntity;
            return body;
        }
        if (isCodeInRange("401", response.httpStatusCode)) {
            throw new ApiException<string>(response.httpStatusCode, "Unauthorized");
