...
export class AliasControllerApiRequestFactory extends BaseAPIRequestFactory {
     ...
     */
    public async getAliasEmails(aliasId: string, page?: number, size?: number, sort?: 'ASC' | 'DESC', options?: Configuration): Promise<RequestContext> {
        let config = options || this.configuration;
     ...
     * @param sort Optional createdAt sort direction ASC or DESC
     */
    public async getAliasThreads(aliasId: string, page?: number, size?: number, sort?: 'ASC' | 'DESC', options?: Configuration): Promise<RequestContext> {
        let config = options || this.configuration;
     ...
     * @param sort Optional createdAt sort direction ASC or DESC
     */
    public async getAliases(page?: number, size?: number, sort?: 'ASC' | 'DESC', options?: Configuration): Promise<RequestContext> {
        let config = options || this.configuration;
...
export class AliasControllerApiResponseProcessor {
     ...
     * @throws ApiException if the response code was not in [200, 299]
     */
     public async updateAlias(response: ResponseContext): Promise<void | Alias > {
        const contentType = ObjectSerializer.normalizeMediaType(response.headers["content-type"]);
        if (isCodeInRange("201", response.httpStatusCode)) {
            return;
        }
        ...
        // Work around for missing responses in specification, e.g. for petstore.yaml
        if (response.httpStatusCode >= 200 && response.httpStatusCode <= 299) {
            const body: void | Alias = ObjectSerializer.deserialize(
                ObjectSerializer.parse(await response.body.text(), contentType),
                "void | Alias", ""
            ) as void | Alias;
            return body;
