...
export class MissedEmailControllerApiRequestFactory extends BaseAPIRequestFactory {
     ...
     * @param sort Optional createdAt sort direction ASC or DESC
     */
    public async getAllUnknownMissedEmails(inboxId?: string, page?: number, searchFilter?: string, size?: number, sort?: 'ASC' | 'DESC', options?: Configuration): Promise<RequestContext> {
        let config = options || this.configuration;
     ...
     */
    public async waitForNthMissedEmail(inboxId?: string, index?: number, timeout?: number, options?: Configuration): Promise<RequestContext> {
        let config = options || this.configuration;
