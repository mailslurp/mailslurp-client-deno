...
export class MissedEmailControllerApiResponseProcessor {
     ...
     * @throws ApiException if the response code was not in [200, 299]
     */
     public async getMissedEmail(response: ResponseContext): Promise<MissedEmailDto > {
        const contentType = ObjectSerializer.normalizeMediaType(response.headers["content-type"]);
        if (isCodeInRange("200", response.httpStatusCode)) {
            const body: MissedEmailDto = ObjectSerializer.deserialize(
                ObjectSerializer.parse(await response.body.text(), contentType),
                "MissedEmailDto", ""
            ) as MissedEmailDto;
            return body;
     ...
     * @throws ApiException if the response code was not in [200, 299]
     */
     public async waitForNthMissedEmail(response: ResponseContext): Promise<MissedEmailDto > {
        const contentType = ObjectSerializer.normalizeMediaType(response.headers["content-type"]);
        if (isCodeInRange("200", response.httpStatusCode)) {
            const body: MissedEmailDto = ObjectSerializer.deserialize(
                ObjectSerializer.parse(await response.body.text(), contentType),
