...
export class InboxReplierControllerApiRequestFactory extends BaseAPIRequestFactory {
     ...
     */
    public async getAllInboxReplierEvents(inboxReplierId?: string, inboxId?: string, emailId?: string, sentId?: string, page?: number, size?: number, sort?: 'ASC' | 'DESC', options?: Configuration): Promise<RequestContext> {
        let config = options || this.configuration;
        ...
        // Path Params
        const localVarPath = '/repliers/events';
        ...
        // Query Params
        if (inboxReplierId !== undefined) {
            requestContext.setQueryParam("inboxReplierId", ObjectSerializer.serialize(inboxReplierId, "string", "uuid"));
        }
        if (inboxId !== undefined) {
            requestContext.setQueryParam("inboxId", ObjectSerializer.serialize(inboxId, "string", "uuid"));
        }
...
export class InboxReplierControllerApiResponseProcessor {
     ...
     * to the expected objects
     *
     * @params response Response returned by the server for a request to getAllInboxReplierEvents
     * @throws ApiException if the response code was not in [200, 299]
     */
     public async getAllInboxReplierEvents(response: ResponseContext): Promise<PageInboxReplierEvents > {
        const contentType = ObjectSerializer.normalizeMediaType(response.headers["content-type"]);
        if (isCodeInRange("200", response.httpStatusCode)) {
