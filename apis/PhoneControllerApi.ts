...
export class PhoneControllerApiRequestFactory extends BaseAPIRequestFactory {
     ...
     */
    public async getPhoneNumbers(phoneCountry?: 'US' | 'GB', page?: number, size?: number, sort?: 'ASC' | 'DESC', since?: Date, before?: Date, options?: Configuration): Promise<RequestContext> {
        let config = options || this.configuration;
        ...
        if (phoneCountry !== undefined) {
            requestContext.setQueryParam("phoneCountry", ObjectSerializer.serialize(phoneCountry, "'US' | 'GB'", ""));
        }
        if (page !== undefined) {
