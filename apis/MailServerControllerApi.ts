...
export class MailServerControllerApiRequestFactory extends BaseAPIRequestFactory {
     ...
     */
    public async getDnsLookups(dNSLookupsOptions: DNSLookupsOptions, options?: Configuration): Promise<RequestContext> {
        let config = options || this.configuration;

        // verify required parameter 'dNSLookupsOptions' is not null or undefined
        if (dNSLookupsOptions === null || dNSLookupsOptions === undefined) {
            throw new RequiredError('Required parameter dNSLookupsOptions was null or undefined when calling getDnsLookups.');
        }


        // Path Params
        const localVarPath = '/mail-server/describe/dns-lookups';
        ...
        requestContext.setHeaderParam("Content-Type", contentType);
        const serializedBody = ObjectSerializer.stringify(
            ObjectSerializer.serialize(dNSLookupsOptions, "DNSLookupsOptions", ""),
            contentType
        );
        requestContext.setBody(serializedBody);
...
export class MailServerControllerApiResponseProcessor {
     ...
     * to the expected objects
     *
     * @params response Response returned by the server for a request to getDnsLookups
     * @throws ApiException if the response code was not in [200, 299]
     */
     public async getDnsLookups(response: ResponseContext): Promise<DNSLookupResults > {
        const contentType = ObjectSerializer.normalizeMediaType(response.headers["content-type"]);
        if (isCodeInRange("200", response.httpStatusCode)) {
            const body: DNSLookupResults = ObjectSerializer.deserialize(
                ObjectSerializer.parse(await response.body.text(), contentType),
