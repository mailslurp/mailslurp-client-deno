...
export class InboxRulesetControllerApiRequestFactory extends BaseAPIRequestFactory {
     ...
     */
    public async getInboxRulesets(before?: Date, inboxId?: string, page?: number, searchFilter?: string, since?: Date, size?: number, sort?: 'ASC' | 'DESC', options?: Configuration): Promise<RequestContext> {
        let config = options || this.configuration;
...
export class InboxRulesetControllerApiResponseProcessor {
     ...
     */
     public async createNewInboxRuleset(response: ResponseContext): Promise<void | InboxRulesetDto > {
     ...
     * @throws ApiException if the response code was not in [200, 299]
     */
     public async testInboxRuleset(response: ResponseContext): Promise<InboxRulesetTestResult | void > {
        const contentType = ObjectSerializer.normalizeMediaType(response.headers["content-type"]);
        if (isCodeInRange("200", response.httpStatusCode)) {
        ...
        // Work around for missing responses in specification, e.g. for petstore.yaml
        if (response.httpStatusCode >= 200 && response.httpStatusCode <= 299) {
            const body: InboxRulesetTestResult | void = ObjectSerializer.deserialize(
                ObjectSerializer.parse(await response.body.text(), contentType),
                "InboxRulesetTestResult | void", ""
            ) as InboxRulesetTestResult | void;
            return body;
        }
     ...
     * @throws ApiException if the response code was not in [200, 299]
     */
     public async testInboxRulesetsForInbox(response: ResponseContext): Promise<InboxRulesetTestResult | void > {
        const contentType = ObjectSerializer.normalizeMediaType(response.headers["content-type"]);
        if (isCodeInRange("200", response.httpStatusCode)) {
            const body: InboxRulesetTestResult = ObjectSerializer.deserialize(
                ObjectSerializer.parse(await response.body.text(), contentType),
     ...
     * @throws ApiException if the response code was not in [200, 299]
     */
     public async testNewInboxRuleset(response: ResponseContext): Promise<InboxRulesetTestResult | void > {
        const contentType = ObjectSerializer.normalizeMediaType(response.headers["content-type"]);
        if (isCodeInRange("200", response.httpStatusCode)) {
            const body: InboxRulesetTestResult = ObjectSerializer.deserialize(
                ObjectSerializer.parse(await response.body.text(), contentType),
