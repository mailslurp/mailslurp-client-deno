...
export class InboxRulesetControllerApiRequestFactory extends BaseAPIRequestFactory {
     ...
     */
    public async testInboxRulesetReceiving(testInboxRulesetReceivingOptions: TestInboxRulesetReceivingOptions, options?: Configuration): Promise<RequestContext> {
        let config = options || this.configuration;

        // verify required parameter 'testInboxRulesetReceivingOptions' is not null or undefined
        if (testInboxRulesetReceivingOptions === null || testInboxRulesetReceivingOptions === undefined) {
            throw new RequiredError('Required parameter testInboxRulesetReceivingOptions was null or undefined when calling testInboxRulesetReceiving.');
        }
        ...
        requestContext.setHeaderParam("Content-Type", contentType);
        const serializedBody = ObjectSerializer.stringify(
            ObjectSerializer.serialize(testInboxRulesetReceivingOptions, "TestInboxRulesetReceivingOptions", ""),
            contentType
        );
        requestContext.setBody(serializedBody);
     ...
     */
    public async testInboxRulesetSending(testInboxRulesetSendingOptions: TestInboxRulesetSendingOptions, options?: Configuration): Promise<RequestContext> {
        let config = options || this.configuration;
        ...
        if (testInboxRulesetSendingOptions === null || testInboxRulesetSendingOptions === undefined) {
            throw new RequiredError('Required parameter testInboxRulesetSendingOptions was null or undefined when calling testInboxRulesetSending.');
        }
...
export class InboxRulesetControllerApiResponseProcessor {
     ...
     * to the expected objects
     *
     * @params response Response returned by the server for a request to testInboxRulesetReceiving
     * @throws ApiException if the response code was not in [200, 299]
     */
     public async testInboxRulesetReceiving(response: ResponseContext): Promise<TestInboxRulesetReceivingResult > {
        const contentType = ObjectSerializer.normalizeMediaType(response.headers["content-type"]);
        if (isCodeInRange("200", response.httpStatusCode)) {
            const body: TestInboxRulesetReceivingResult = ObjectSerializer.deserialize(
                ObjectSerializer.parse(await response.body.text(), contentType),
                "TestInboxRulesetReceivingResult", ""
            ) as TestInboxRulesetReceivingResult;
            return body;
        }
     ...
     * to the expected objects
     *
     * @params response Response returned by the server for a request to testInboxRulesetSending
     * @throws ApiException if the response code was not in [200, 299]
     */
     public async testInboxRulesetSending(response: ResponseContext): Promise<TestInboxRulesetSendingResult > {
        const contentType = ObjectSerializer.normalizeMediaType(response.headers["content-type"]);
        if (isCodeInRange("200", response.httpStatusCode)) {
            const body: TestInboxRulesetSendingResult = ObjectSerializer.deserialize(
                ObjectSerializer.parse(await response.body.text(), contentType),
                "TestInboxRulesetSendingResult", ""
            ) as TestInboxRulesetSendingResult;
            return body;
        }
